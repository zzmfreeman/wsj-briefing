...
{{
  "bullets": [
    "核心事实：<50-150字，最关键的事实、数字、结论，需具体翔实>",
    "关键细节：<50-150字，最重要的背景、细节或数据，需具体翔实>",
    "影响：<50-150字，对谁有什么具体影响，因果链条清晰>"
  ],
...
要求：
1. bullets 必须恰好3条，按顺序分别以"核心事实："、"关键细节："、"影响："开头，格式不可省略
2. 每段50-150字，可多句，内容翔实：含具体数字、人名、机构名、因果关系，不要空泛概括
3. insight 只有一句话，简短、犀利、触达本质，不复述摘要
4. 直接陈述事实，不要用"据报道""报道称"等转述词
5. 不要使用markdown链接格式
6. importance是1-5分重要度：5=头版头条/重大突破，4=重要政策/大公司动态，3=行业重要变化，2=常规新闻，1=花边/观点专栏"""
